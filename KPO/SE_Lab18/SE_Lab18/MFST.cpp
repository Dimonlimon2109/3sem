...
namespace MFST {
	...
	bool Mfst::savestate() {
		storestate.push(MfstState(lenta_position, st, nrulechain));
		return true;
	}
	...
	Mfst::RC_STEP Mfst::step() {
		...
		std::cout << std::endl;
		std::cout << std::setw(4) << std::left << counter << ": ";
		*log.stream << std::endl;
		*log.stream << std::setw(4) << std::left << counter << ": ";
		if (lenta_position < lenta_size) {
			if (GRB::Rule::Chain::isN(st.top())) {
				...
				if (nrule >= 0) {
					...
					if (nrulechain >= 0) {
						...
						std::cout << std::setw(20) << std::left << buff;
						*log.stream << std::setw(20) << std::left << buff;
						getCLenta(buff, lenta_position);
						std::cout << std::setw(30) << std::left << buff;
						*log.stream << std::setw(30) << std::left << buff;
						getCSt(buff);
						std::cout << std::setw(20) << std::left << buff << std::endl;
						...
						*log.stream << std::setw(4) << std::left << counter << ": " << std::setw(20) << std::left << "SAVESTATE:" << std::setw(30) << std::left << saveCounter++;
						std::cout << std::setw(4) << std::left << counter << ": " << std::setw(20) << std::left << "SAVESTATE:" << std::setw(30) << std::left << saveCounter;
						st.pop();
						push_chain(chain);
						state = NS_OK;
					} else {
						...
						if (reststate()) {
							...
							std::cout << std::setw(20) << std::left << "TS_NOK/NS_NORULECHAIN" << std::endl << std::setw(4) << std::left << counter << ": " << std::setw(20) << std::left << "RESTSTATE";
			...
			state = LENTA_END;
			std::cout << std::setw(20) << std::left << "LENTA_END";
	...
	bool Mfst::start() {
		std::cout << std::endl << std::setw(4) << std::left << "Шаг" << ": " << std::setw(20) << std::left << "Правило" << std::setw(30) << std::left << "Входная лента" << std::setw(20) << std::left << "Стек" << std::endl;
		*log.stream << std::endl << std::setw(4) << std::left << "Шаг" << ": " << std::setw(20) << std::left << "Правило"  << std::setw(30) << std::left << "Входная лента" << std::setw(20) << std::left << "Стек" << std::endl;
		...
		case MFST::Mfst::NS_NORULE: {
			std::cout << std::setw(20) << std::left << "------>NO_RULE" << std::endl;
			std::cout << "-------------------------------------------------------------------------------------" << std::endl;
			std::cout << getDiagnosis(0, buff) << std::endl;
			std::cout << getDiagnosis(1, buff) << std::endl;
			std::cout << getDiagnosis(2, buff) << std::endl;
		...
		case MFST::Mfst::NS_NORULECHAIN: {
			std::cout << std::setw(20) << std::left << "------>NO_RULECHAIN";
			*log.stream << std::setw(20) << std::left << "------>NO_RULECHAIN";
			break;
		}
		case MFST::Mfst::NS_ERROR: {
			std::cout << std::setw(20) << std::left << "------>NS_ERROR";
			*log.stream << std::setw(20) << std::left << "------>NS_ERROR";
			break;
		}
		case MFST::Mfst::LENTA_END: {
			std::cout << std::setw(4) << std::left << counter << std::setw(20) << std::left << "------>LENTA_END" << std::endl;
			std::cout << "-------------------------------------------------------------------------------------" << std::endl;
		...
		case MFST::Mfst::SURPRISE: {
			std::cout << std::setw(20) << std::left << "------>SURPRISE ";
