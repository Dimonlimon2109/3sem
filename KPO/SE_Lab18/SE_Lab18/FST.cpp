		...
		for (int i = 0; i < lexemsSize; i++) {
			...
			for (int j = 0; j < FSTarrayLen; j++) {
				if (execute(*FSTarray[j].fst)) {
					...
					/*if (LTObj.lexema == LEX_PLUS || LTObj.lexema == LEX_LEFTTHESIS || LTObj.lexema == LEX_RIGHTTHESIS || LTObj.lexema == LEX_LEFTBRACE || LTObj.lexema == LEX_RIGHTBRACE) {
			...
			if (!isExecuted) {
				throw ERROR_THROW_IN(119, lex.line, lex.col);
			}
		}	
		if (!isWasMain) {
