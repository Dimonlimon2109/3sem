...
namespace Log {
	...
	void WriteTables(LOG log, IT::IdTable& idtable, LT::LexTable& lextable) {
		...
		for (int i = 0; i < idtable.size; i++) {
			(*log.stream) << idtable.table[i].id << " Первое вхождение в таблице лексем: " << idtable.table[i].idxfirstLE << " Тип: " << idtable.table[i].idtype << " Тип данных: " << idtable.table[i].iddatatype << "\n";
		}
	}
